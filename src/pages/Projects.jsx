...
export default function Projects({ user, setUser, onLogout }) {
  // Tasks state
  ...
  const handleApiError = (err, defaultMsg) => {
    ...
    if (message.toLowerCase().includes('session expired') || message.toLowerCase().includes('unauthorized') || message.toLowerCase().includes('token')) {
  ...
  // 1. READ TASKS - Fetch tasks on mount when user is authenticated
  ...
  return (
    <div className="bg-background dark:bg-inverse-surface/10 py-16 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto duration-300 min-h-[85vh]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 dark:bg-primary/20 text-primary dark:text-[#5c8bee] rounded-full mb-3 text-xs font-bold">
            ...
            Backend API: http://localhost:5000 (JWT Secured)
          </div>
          ...
          <p className="font-body-md text-on-surface-variant dark:text-gray-300 text-sm max-w-xl">
            Practical 7: Full-stack JWT authentication, validation middleware pipeline, and user-isolated task persistence.
          </p>
        </div>
        ...
        {user && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2.5 bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/30 hover:bg-red-500/20 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
            >
      ...
      {/* RENDER AUTH COMPONENT IF UNAUTHENTICATED */}
      {!user ? (
        <Auth 
          onAuthSuccess={(userData) => {
            setUser(userData);
            showNotification('Authentication successful! Loading workspace...', 'success');
          }}
          showNotification={showNotification}
        />
      ) : (
        /* RENDER TASK DASHBOARD IF AUTHENTICATED */
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  required
                  placeholder="e.g. Practical 7 Middleware Validation"
                  value={title}
